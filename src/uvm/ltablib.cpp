...
static int tappend(lua_State *L) {
	...
	lua_Integer pos;  /* where to insert new element */
	if(lua_gettop(L)<2)
		return luaL_error(L, "wrong number of arguments to 'append'");
	pos = e;
	lua_seti(L, 1, pos);  /* t[pos] = v */
	return 0;
}
...
static int tlength(lua_State *L)
{
	if (lua_gettop(L) < 1 || !lua_istable(L, 1))
		return luaL_error(L, "only array/table can get length");
	auto arraylen = luaL_len(L, 1);
	lua_pushinteger(L, arraylen);
	return 1;
}
