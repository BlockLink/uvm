...
namespace uvm
{
    namespace lua
    {
      namespace api
      {
		...
		std::map<std::string, lua_CFunction> chain_apis;
	  ...
	  using uvm::lua::api::chain_apis;
		namespace lib
		{
			...
			std::set<std::string> global_whitelist = {
				"print", "pprint", "table", "string", "time", "math", "safemath", "json", "type", "require", "Array", "Stream",
				"import_contract_from_address", "import_contract",  "delegate_call", "in_delegate_call",
				...
				"caller", "caller_address", "fast_map_get", "fast_map_set",
				...
				"setmetatable",
				"hex_to_bytes", "bytes_to_hex", "sha256_hex", "sha1_hex", "sha3_hex", "ripemd160_hex", "str_to_hex",
				"cbor_encode", "cbor_decode", "send_message"
			};
				...
				*/
                ...
                if (use_contract)
                {
					...
					*/
					...
					for (auto iter = chain_apis.begin(); iter != chain_apis.end(); iter++) {
						add_global_c_function(L, iter->first.c_str(), iter->second);
	...
	storage: S
            ...
            bool check_contract_proto(lua_State *L, uvm_types::GcProto *proto, char *error, std::list<uvm_types::GcProto*> *parents)
            {
                ...
                for (pc = 0; pc < code_size; pc++)
                {
                    ...
                    case UOP_GETUPVAL:
                    {
						...
						if (global_whitelist.find(std::string(cname)) != global_whitelist.end()) {
							in_whitelist = true;
						}
                    ...
                    case UOP_GETTABUP:
                    {
                        ...
                        if (ISK(c)){
                            ...
                            bool in_whitelist = false;
							if (global_whitelist.find(std::string(cname)) != global_whitelist.end()|| strcmp(cname, "-") == 0) {
								in_whitelist = true;
							}
            ...
            */
			...
			void add_api_to_uvm_white_list(std::string apiname) {
				//globalvar_whitelist
				//for(int i=0;i<)
				if (global_whitelist.find(apiname) == global_whitelist.end()) {
					global_whitelist.insert(apiname);
				}
