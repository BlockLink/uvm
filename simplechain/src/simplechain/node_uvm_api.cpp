...
namespace simplechain {
	...
	*/
	bool NodeUvmChainApi::has_exception(lua_State *L)
	{
	...
	*/
	void NodeUvmChainApi::clear_exceptions(lua_State *L)
	{
		has_error = 0;
	}
	...
	*/
	void NodeUvmChainApi::throw_exception(lua_State *L, int code, const char *error_format, ...)
	{
		if (has_error)
			return;
		has_error = 1;
	...
	*/
	int NodeUvmChainApi::check_contract_api_instructions_over_limit(lua_State *L)
	{
		try {
	...
	static std::shared_ptr<uvm::blockchain::Code> get_contract_code_by_id(evaluate_state* evaluator, const std::string& contract_id) {
		try {
			if (evaluator) {
				auto contract = Data_getter::get_contract_object(contract_id.c_str());
				if (contract) {
	...
	*/
	static bool contains_contract_by_id( evaluate_state* evaluator, const std::string& contract_id) {
		try {
			if (evaluator) {
				if (Data_getter::get_contract_info(contract_id.c_str()) != nullptr) {
					return true;
				}
			}
			return false;
	...
	*/
	...
	static std::shared_ptr<UvmContractInfo> get_contract_info_by_id(evaluate_state* evaluator, const std::string& contract_id) {
		try {
			if (evaluator) {
				return Data_getter::get_contract_info(contract_id.c_str());
			}
			return nullptr;
		}FC_CAPTURE_AND_LOG((contract_id))
			return nullptr;
	...
	//not used
	int NodeUvmChainApi::get_stored_contract_info(lua_State *L, const char *name, std::shared_ptr<UvmContractInfo> contract_info_ret)
	{
		auto evaluator = get_contract_evaluator(L);
	...
	//// change....
	int NodeUvmChainApi::get_stored_contract_info_by_address(lua_State *L, const char *contract_id, std::shared_ptr<UvmContractInfo> contract_info_ret)
	{

		auto info = Data_getter::get_contract_info(contract_id);
		if (info == nullptr) {
			return 0;
		}
		contract_info_ret->contract_apis.clear();

		std::copy(info->contract_apis.begin(), info->contract_apis.end(), std::back_inserter(contract_info_ret->contract_apis));

		contract_info_ret->contract_api_arg_types.clear();
		for (const auto&item : info->contract_api_arg_types) {
	...
	std::shared_ptr<UvmModuleByteStream> NodeUvmChainApi::get_bytestream_from_code(lua_State *L, const uvm::blockchain::Code& code)
	{
	...
	//check?
	void NodeUvmChainApi::get_contract_address_by_name(lua_State *L, const char *name, char *address, size_t *address_size)
	{
		auto evaluator = get_contract_evaluator(L);
	...
	//change
	/*
	bool NodeUvmChainApi::check_contract_exist_by_address(lua_State *L, const char *address)
	{
	auto evaluator = get_contract_evaluator(L);
	...
	*/
	...
	bool NodeUvmChainApi::check_contract_exist(lua_State *L, const char *name)
	{
		auto evaluator = get_contract_evaluator(L);
	...
	*/
	//check, not used for contract call
	std::shared_ptr<UvmModuleByteStream> NodeUvmChainApi::open_contract(lua_State *L, const char *name)
	{
	...
	//change
	std::shared_ptr<UvmModuleByteStream> NodeUvmChainApi::open_contract_by_address(lua_State *L, const char *address)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		auto code = Data_getter::get_contract_code(address);
		if (code == nullptr) {
			return nullptr;
		}
		try
	...
	//not used
	UvmStorageValue NodeUvmChainApi::get_storage_value_from_uvm(lua_State *L, const char *contract_name, const std::string& name,
	...
	bool NodeUvmChainApi::commit_storage_changes_to_uvm(lua_State *L, AllContractsChangesMap &changes)
	{
		auto evaluator = get_contract_evaluator(L);
	...
	intptr_t NodeUvmChainApi::register_object_in_pool(lua_State *L, intptr_t object_addr, UvmOutsideObjectTypes type)
	{
	...
	intptr_t NodeUvmChainApi::is_object_in_pool(lua_State *L, intptr_t object_key, UvmOutsideObjectTypes type)
	{
		auto node = uvm::lua::lib::get_lua_state_value_node(L, GLUA_OUTSIDE_OBJECT_POOLS_KEY);
		// Map<type, Map<object_key, object_addr>>
		std::map<UvmOutsideObjectTypes, std::shared_ptr<std::map<intptr_t, intptr_t>>> *object_pools = nullptr;
	...
	void NodeUvmChainApi::release_objects_in_pool(lua_State *L)
	{
		auto node = uvm::lua::lib::get_lua_state_value_node(L, GLUA_OUTSIDE_OBJECT_POOLS_KEY);
		// Map<type, Map<object_key, object_addr>>
		std::map<UvmOutsideObjectTypes, std::shared_ptr<std::map<intptr_t, intptr_t>>> *object_pools = nullptr;
	...
	bool NodeUvmChainApi::register_storage(lua_State *L, const char *contract_name, const char *name)
	{
		return true;
	}
	...
	lua_Integer NodeUvmChainApi::transfer_from_contract_to_public_account(lua_State *L, const char *contract_address, const char *to_account_name,
		const char *asset_type, int64_t amount)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
	...
	int64_t NodeUvmChainApi::get_contract_balance_amount(lua_State *L, const char *contract_address, const char* asset_symbol)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
	...
	int64_t NodeUvmChainApi::get_transaction_fee(lua_State *L)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		try {
			return 0;
	...
	uint32_t NodeUvmChainApi::get_chain_now(lua_State *L)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		try {
			auto evaluator = get_contract_evaluator(L);
	...
	uint32_t NodeUvmChainApi::get_chain_random(lua_State *L)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		try {
			auto evaluator = get_contract_evaluator(L);
	...
	uint32_t NodeUvmChainApi::get_chain_safe_random(lua_State *L, bool diff_in_diff_txs) {
		return get_chain_random(L);
	}

	std::string NodeUvmChainApi::get_transaction_id(lua_State *L)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		return get_transaction_id_without_gas(L);
	}

	std::string NodeUvmChainApi::get_transaction_id_without_gas(lua_State *L) const {
		try {
			auto evaluator = get_contract_evaluator(L);
	...
	uint32_t NodeUvmChainApi::get_header_block_num(lua_State *L)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		try {
			auto evaluator = get_contract_evaluator(L);
	...
	uint32_t NodeUvmChainApi::get_header_block_num_without_gas(lua_State *L) const
	{
		try {
			auto evaluator = get_contract_evaluator(L);
	...
	uint32_t NodeUvmChainApi::wait_for_future_random(lua_State *L, int next)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		try {
			auto evaluator = get_contract_evaluator(L);
	...
	int32_t NodeUvmChainApi::get_waited(lua_State *L, uint32_t num)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		try {
			if (num <= 1)
	...
	void NodeUvmChainApi::emit(lua_State *L, const char* contract_id, const char* event_name, const char* event_param)
	{
		uvm::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
		try {
			auto evaluator = get_contract_evaluator(L);
	...
	//change 
	bool NodeUvmChainApi::is_valid_address(lua_State *L, const char *address_str)
	{
		/*
		...
		*/
		std::string addr(address_str);
		if (addr.find("HX") == 0 && addr.size() > strlen("HX")) {
			return true;
		}
		return helper::is_valid_address(addr);
	}
	...
	//change 
	bool NodeUvmChainApi::is_valid_contract_address(lua_State *L, const char *address_str)
	{
		/*
		std::string addr(address_str);
		...
		*/
		std::string addr(address_str);
		if (addr.find("HXC") == 0 && addr.size() > strlen("HXC")) {
			return true;
		}
		return helper::is_valid_contract_address(addr);
	...
	//change
	const char * NodeUvmChainApi::get_system_asset_symbol(lua_State *L)
	{
		auto evaluator = get_contract_evaluator(L);
		UNUSED(evaluator);
	...
	uint64_t NodeUvmChainApi::get_system_asset_precision(lua_State *L)
	{
	...
	std::vector<unsigned char> NodeUvmChainApi::hex_to_bytes(const std::string& hex_string) {
		const auto& chars = hex_to_chars(hex_string);
		std::vector<unsigned char> bytes(chars.size());
		memcpy(bytes.data(), chars.data(), chars.size());
		return bytes;
	}
	std::string NodeUvmChainApi::bytes_to_hex(std::vector<unsigned char> bytes) {
		std::vector<char> chars(bytes.size());
		memcpy(chars.data(), bytes.data(), bytes.size());
		return fc::to_hex(chars);
	}
	std::string NodeUvmChainApi::sha256_hex(const std::string& hex_string) {
		const auto& chars = hex_to_chars(hex_string);
		auto hash_result = fc::sha256::hash(chars.data(), static_cast<uint32_t>(chars.size()));
		return hash_result.str();
	}
	std::string NodeUvmChainApi::sha1_hex(const std::string& hex_string) {
		const auto& chars = hex_to_chars(hex_string);
		auto hash_result = fc::sha1::hash(chars.data(), static_cast<uint32_t>(chars.size()));
		return hash_result.str();
	}
	std::string NodeUvmChainApi::sha3_hex(const std::string& hex_string) {
		Keccak keccak(Keccak::Keccak256);
		const auto& chars = hex_to_chars(hex_string);
		auto hash_result = keccak(chars.data(), chars.size());
		return hash_result;
	}
	std::string NodeUvmChainApi::ripemd160_hex(const std::string& hex_string) {
		const auto& chars = hex_to_chars(hex_string);
		auto hash_result = fc::ripemd160::hash(chars.data(), static_cast<uint32_t>(chars.size()));
		return hash_result.str();
	}

	std::string NodeUvmChainApi::get_address_role(lua_State* L, const std::string& addr) {
		return std::string(addr);
	}

	int64_t NodeUvmChainApi::get_fork_height(lua_State* L, const std::string& fork_key) {
		return 1;
	}

	bool NodeUvmChainApi::use_cbor_diff(lua_State* L) const {
	...
	std::string NodeUvmChainApi::pubkey_to_address_string(const fc::ecc::public_key& pub) const {
	...
	bool NodeUvmChainApi::use_gas_log(lua_State* L) const {
		const auto& txid = get_transaction_id_without_gas(L);
		return false;
	}
	bool NodeUvmChainApi::use_step_log(lua_State* L) const {
		const auto& txid = get_transaction_id_without_gas(L);
		return false;
	}
	...
	void NodeUvmChainApi::before_contract_invoke(lua_State* L, const std::string& contract_addr, const std::string& txid) {
		if (true)
			return;
	...
	void NodeUvmChainApi::dump_contract_state(lua_State* L, const std::string& contract_addr, const std::string& txid, std::ostream& out) {
		try {
			auto evaluator = get_contract_evaluator(L);
